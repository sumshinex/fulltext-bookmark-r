...
function buildSearchEngineRulePrompt(context: SearchEnginePageContext): string {
  return [
    ...
    "Requirements:",
    "- urlPattern must be a valid regular expression string that matches the current page URL.",
    "- Prefer using values that already appear in the page context candidate lists.",
    "- queryParam should come from queryParamCandidates when available.",
    "- Only use queryInputSelector when queryParamCandidates is empty or clearly unsuitable.",
    "- containerSelector or containerId should come from containerSelectorCandidates or containerIdCandidates.",
    "- Prefer the first plausible high-signal candidate from the context instead of inventing a new selector.",
    "- Use pageTextHints only to judge which provided candidates are most plausible.",
...
function buildSearchEngineRuleCorrectionPrompt(input: {
  ...
  return [
    ...
    "Requirements:",
    "- Keep fields that already look correct unless diagnostics or context candidates indicate a better replacement.",
    "- If URL did not match, refine urlPattern so it matches the current page URL directly.",
    "- If container did not match, replace containerSelector/containerId with a value from diagnostics suggestions or page context candidates.",
    "- If query did not match, replace queryParam/queryInputSelector with a value from diagnostics suggestions or page context candidates.",
    "- Prefer queryParam from queryParamCandidates when available.",
    "- Only use queryInputSelector when queryParamCandidates is empty or clearly unsuitable.",
    "- Prefer containerSelector/containerId that already exists in the candidate lists; do not invent a new selector unless diagnostics explicitly require it.",
    "- include only one of containerId/containerSelector when possible.",
    "- include only one of queryParam/queryInputSelector when possible.",
    "- insertPosition should usually be prepend.",
    "Current page context:",
